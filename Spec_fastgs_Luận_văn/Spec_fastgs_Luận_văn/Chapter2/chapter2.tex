...
\begin{xltabular}{\textwidth}
    ...
    Cải tiến thuật toán \textbf{sắp xếp tile và kiểm soát mật độ thích ứng (ADC)}; lược bỏ các bước kiểm tra trùng lặp không cần thiết và tối ưu lịch trình nhân bản / cắt tỉa Gaussian. &
    Thời gian huấn luyện cực ngắn (< 2 phút trên Mip-NeRF 360); tốc độ kết xuất vượt trội (> 570 FPS); giữ nguyên chất lượng tổng quan trên các bề mặt khuếch tán. &
    Ngầm giả định bề mặt khuếch tán (Lambertian); chất lượng sụt giảm trên các vùng phản xạ gương tần số cao hoặc có vệt sáng biến đổi mạnh theo góc nhìn. \\ \hline

    \textbf{Speedy Splat} \cite{hanson2024speedysplat} &
    Tối ưu hóa hạt nhân rasterization trên GPU; kết hợp \textbf{cắt tỉa thành phần thuộc tính} và loại bỏ các Gaussian đóng góp độ truyền qua mờ nhạt. &
    Tốc độ kết xuất kỷ lục gần 900 FPS; giảm dung lượng bộ nhớ lưu trữ mô hình đáng kể. &
    Loại bỏ bớt thông tin khiến các chi tiết ngoại quan tần số cao bị mờ nhẹ; khả năng mô hình hóa vệt sáng phản xạ phức tạp bị hạn chế. \\ \hline

    \textbf{Taming 3DGS} \cite{mallick2024taming} &
    Ràng buộc ngân sách Gaussian tối đa thông qua \textbf{cơ chế kiểm soát mật độ dựa trên entropy} và đo đạc đóng góp hình học; tự động loại bỏ các hạt thừa tại vùng rỗng. &
    Kiểm soát chặt chẽ dung lượng bộ nhớ và số hạt 3DGS; huấn luyện ổn định và kết xuất nhanh chóng. &
    Ràng buộc số lượng hạt cứng làm suy giảm độ chi tiết tại các vùng bề mặt vật liệu phức tạp đòi hỏi nhiều hạt biểu diễn. \\
    \hline
    \caption{Tóm tắt các công trình cải thiện hiệu suất kết xuất và tối ưu hóa tài nguyên}
    \label{tab:rw-speed} \\
\end{xltabular}
...
\paragraph{FastGS: Tối ưu hóa cơ chế kiểm soát mật độ và sắp xếp Tile}
\label{para:fastgs-detail}
FastGS \cite{ren2025fastgs} tiếp cận bài toán tối ưu hóa 3DGS dựa trên nhận định rằng quá trình kiểm soát mật độ thích ứng (ADC) và sắp xếp tile trong 3DGS nguyên bản chứa nhiều phép tính thừa. Bằng cách thiết lập lịch trình cắt tỉa thông minh và cải tiến thuật toán rasterization, FastGS giảm thời gian huấn luyện từ 20--30 phút xuống dưới 2 phút trên tập dữ liệu Mip-NeRF 360 \cite{barron2022mipnerf360}, đồng thời đạt tốc độ kết xuất vượt trội 579 FPS. Tuy nhiên, do FastGS kế thừa hệ số SH bậc thấp và tập trung cắt giảm các hạt trùng lặp, mô hình gặp khó khăn khi tái tạo các bề mặt phản xạ bóng, nơi các vệt sáng yêu cầu mật độ hạt cao và đại lượng biểu diễn quang học sắc nét.

\paragraph{Speedy Splat: Cắt giảm thành phần và kết xuất siêu tốc}
\label{para:speedysplat-detail}
Speedy Splat \cite{hanson2024speedysplat} tập trung vào việc đẩy tốc độ kết xuất thời gian thực lên ngưỡng kỷ lục. Phương pháp tối ưu lại toàn bộ hạt nhân GPU rasterization và loại bỏ các Gaussian có đóng góp độ truyền qua nhỏ hơn ngưỡng cực nhỏ ($T_i \alpha_i < \epsilon$). Kỹ thuật này giúp Speedy Splat đạt tốc độ kết xuất lên tới 898 FPS trên tập Mip-NeRF 360. Tuy nhiên, việc cắt giảm bớt các thuộc tính ngoại quan khiến cho các vùng phản xạ gương nhạy cảm bị mất chi tiết và dễ xuất hiện hiện tượng mờ nhạt.

\paragraph{Taming 3DGS: Kiểm soát số lượng Gaussian thích ứng}
\label{para:taming3dgs-detail}
Taming 3DGS \cite{mallick2024taming} giải quyết vấn đề bùng nổ số lượng Gaussian trong 3DGS nguyên bản bằng cách đưa vào một cơ chế quản lý ngân sách mật độ thích ứng dựa trên entropy. Mô hình đo lường mức độ đóng góp thông tin của từng Gaussian và loại bỏ sớm các hạt rác hoặc hạt dư thừa. Nhờ đó, Taming 3DGS duy trì được số lượng hạt nhỏ gọn (chỉ khoảng 0.63M hạt trên Mip-NeRF 360) mà vẫn giữ được chất lượng tái tạo tốt. Mặc dù vậy, việc giới hạn ngân sách hạt khiến mô hình khó lòng biểu diễn các bề mặt phản xạ phức tạp vốn yêu cầu sự phối hợp của nhiều Gaussian chồng lấp.
