...
def render_set(model_path, load2gpt_on_the_fly, name, iteration, views, gaussians, pipeline, background, specular,
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        results = render(view, gaussians, pipeline, background, mlp_color, hybrid=hybrid,
                         voxel_visible_mask=voxel_visible_mask)

        # Render base image without MLP color for diagnostics
        base_results = render(view, gaussians, pipeline, background, torch.zeros_like(mlp_color), hybrid=hybrid,
                              voxel_visible_mask=voxel_visible_mask)
        base_image = base_results["render"]
        ...
        only_asg = torch.clamp(rendering - base_image, min=0.0)
        residual_real = torch.clamp(gt - base_image, min=0.0)
        ...
        torchvision.utils.save_image(only_asg, os.path.join(spec_path, '{0:05d}_only_asg.png'.format(idx)))
        torchvision.utils.save_image(residual_real, os.path.join(spec_path, '{0:05d}_residual_real.png'.format(idx)))
    ...
    # Discard five warm-up frames when the test set is large enough.
    timed_frames = t_list[5:] if len(t_list) > 5 else t_list
    t = np.array(timed_frames)
    fps = 1.0 / t.mean() if len(t) > 0 else 0.0
...
def render_video(model_path, iteration, views, gaussians, pipeline, background, specular, hybrid=True):
    ...
    for idx, pose in enumerate(tqdm(generate_ellipse_path(views, n_frames=600), desc="Rendering progress")):
        ...
        mlp_color = specular.step(gaussians.get_asg_features[voxel_visible_mask],
                                      dir_pp_normalized[voxel_visible_mask], normal[voxel_visible_mask])
        rendering = render(view, gaussians, pipeline, background, mlp_color, hybrid=hybrid,
                           voxel_visible_mask=voxel_visible_mask)["render"]
        renderings.append(to8b(rendering.cpu().numpy()))
        # torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
...
def render_sets(dataset: ModelParams, iteration: int, opt: OptimizationParams, pipeline: PipelineParams,
                skip_train: bool, skip_test: bool, mode: str, hybrid=True):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree, dataset.asg_degree)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False)
        specular = SpecularModel(dataset.asg_degree, dataset.is_real, dataset.is_indoor)
        ...
        if mode == "render":
            ...
            render_video(dataset.model_path, scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline,
                         background, specular, hybrid=hybrid)
            return

        if not skip_train:
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, "train", scene.loaded_iter,
                        scene.getTrainCameras(), gaussians, pipeline,
                        background, specular, use_filter, hybrid=hybrid)

        if not skip_test:
            test_fps = render_func(dataset.model_path, dataset.load2gpu_on_the_fly, "test", scene.loaded_iter,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hybrid", type=str2bool, default=True,
                        help="True renders SH + ASG. False renders ASG/MLP color only.")
    ...
    render_sets(model.extract(args), args.iteration, op.extract(args), pipeline.extract(args), args.skip_train,
                args.skip_test, args.mode, hybrid=args.hybrid)
