...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, hybrid=True, specular_start_iter=3000):
    import time
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if (not hybrid) or iteration > specular_start_iter:
        ...
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, mlp_color, hybrid=hybrid,
        ...
        with torch.no_grad():
            ...
            cur_psnr = training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end),
                                       ...
                                       dataset.load2gpu_on_the_fly, use_filter, hybrid)
    ...
    training_time_formatted = f"{int(training_time_seconds // 60)}m {int(training_time_seconds % 60)}s"
    ...
    train_info = {
        ...
        "git_branch": "Specular-Gaussians_Baseline",
        "image_scale": getattr(dataset, 'images', 'images_8'),
        "iterations": int(opt.iterations),
        "initial_gaussians": int(initial_gaussians),
        ...
        "last_lpips": float(last_lpips.item() if hasattr(last_lpips, 'item') else last_lpips)
    }
    with open(os.path.join(dataset.model_path, "train_info.json"), "w") as f:
...
def test_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc,
                renderArgs, specular_mlp, load2gpu_on_the_fly, use_filter, hybrid=True):
    if tb_writer:
    ...
    if iteration in testing_iterations:
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, viewpoint in enumerate(config['cameras']):
                ...
                image = torch.clamp(
                    renderFunc(viewpoint, scene.gaussians, *renderArgs, mlp_color, hybrid=hybrid,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--specular_start_iter", type=int, default=3000,
                        help="Iteration to start ASG when --hybrid=True. Ignored when --hybrid=False.")
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations,
             hybrid=args.hybrid, specular_start_iter=args.specular_start_iter)
    ...
    render_sets(lp.extract(args), -1, op.extract(args), pp.extract(args), skip_train=True, skip_test=False,
                mode="render", hybrid=args.hybrid)
    print("\nRendering complete.")
