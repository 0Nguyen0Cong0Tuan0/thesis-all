...
cells.append({
    ...
    "source": [
        ...
        "5. **Khâu 4 (Kết xuất đánh giá cuối cùng Iter 30k)**: Thu thập trọn bộ 4 thư mục kết quả đánh giá (`spec`, `renders`, `gt`, `heatmap`) cho 10 scenes đã chọn cố định.\n",
...
# Cell 8: Step 4: Render & Save Final Eval Folders (renders, gt, spec, heatmap) (Code)
cells.append({
    "cell_type": "code",
    "execution_count": None,
    ...
    "outputs": [],
    "source": [
        "# ── KHÂU 4: Kết Xuất Cuối Cùng (Iter 30k) & Thu Thập 4 Thư Mục (spec, renders, gt, heatmap) ────\n",
        "import os\n",
        "import sys\n",
        "import json\n",
        "import shutil\n",
        ...
        "\n",
        "print(\"🎨 Đang chạy render.py để kết xuất bộ ảnh đánh giá cuối cùng (Iteration 30,000)...\" )\n",
        "render_cmd = [\n",
        ...
        "\n",
        "# 1. Đường dẫn thư mục output do render.py tạo ra: output/demo_counter/test/ours_30000\n",
        "ours_30000_dir = os.path.join(model_path, \"test\", \"ours_30000\")\n",
        "\n",
        "if os.path.exists(ours_30000_dir):\n",
        "    print(f\"📦 Đang sao chép toàn bộ 4 thư mục kết quả từ {ours_30000_dir} vào {step4_dir} ...\")\n",
        "    for subfolder in [\"spec\", \"renders\", \"gt\", \"heatmap\"]:\n",
        "        src_sub = os.path.join(ours_30000_dir, subfolder)\n",
        "        dst_sub = os.path.join(step4_dir, subfolder)\n",
        "        if os.path.exists(src_sub):\n",
        "            if os.path.exists(dst_sub):\n",
        "                shutil.rmtree(dst_sub)\n",
        "            shutil.copytree(src_sub, dst_sub)\n",
        "            print(f\"  -> Đã lưu '{subfolder}' ({len(os.listdir(dst_sub))} tệp) -> {dst_sub}\")\n",
        "    \n",
        "    # 2. Trích xuất riêng 10 khung ảnh của 10 fixed camera viewpoints vào folder fixed_10_scenes_renders\n",
        "    fixed_cams_path = os.path.join(DEMO_OUTPUT_DIR, \"fixed_cameras.json\")\n",
        ...
        "        \n",
        "        fixed_renders_dir = os.path.join(step4_dir, \"fixed_10_scenes_renders\")\n",
        "        os.makedirs(fixed_renders_dir, exist_ok=True)\n",
        "        \n",
        "        renders_src = os.path.join(step4_dir, \"renders\")\n",
        "        if os.path.exists(renders_src):\n",
        "            for i, cam_name in enumerate(selected_cams):\n",
        "                found_render = None\n",
        "                for fname in os.listdir(renders_src):\n",
        "                    if cam_name in fname:\n",
        "                        found_render = os.path.join(renders_src, fname)\n",
        "                        break\n",
        "                if found_render:\n",
        "                    out_name = f\"view_{i+1:02d}_{cam_name}.png\"\n",
        "                    shutil.copy2(found_render, os.path.join(fixed_renders_dir, out_name))\n",
        "                    print(f\"  -> Đã lưu Render 10 Scenes fixed: {out_name}\")\n",
        "                    \n",
        "    print(f\"✅ Khâu 4 hoàn tất: Đã lưu trọn bộ kết quả renders (spec, renders, gt, heatmap) tại {step4_dir}!\")\n",
        "else:\n",
...
cells.append({
    ...
    "source": [
        ...
        "                repo_type=\"dataset\"\n",
        "            )\n",
        "            print(f\"🎉 CHÚC MỪNG! Đã upload thành công toàn bộ dữ liệu Video Demo & Render (spec, renders, gt, heatmap) lên Hugging Face!\")\n",
