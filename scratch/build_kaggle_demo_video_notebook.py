...
cells.append({
    ...
    "outputs": [],
    "source": [
        "# ── Cài Đặt Thư Viện Phụ Thuộc & Biên Dịch CUDA Submodules ───────────────────────\n",
...
cells.append({
    ...
    "source": [
        ...
        "\n",
        "# Đọc lại danh sách 10 camera cố định\n",
        "fixed_cams_path = os.path.join(DEMO_OUTPUT_DIR, \"fixed_cameras.json\")\n",
        "with open(fixed_cams_path, \"r\", encoding=\"utf-8\") as f:\n",
        "    cam_data = json.load(f)\n",
        ...
        "\n",
        "# Tạo script huấn luyện tùy chỉnh có callback render 10 views mỗi 1k iter\n",
        "train_runner_script = os.path.join(PROJECT_DIR, \"spec-fastgs\", \"train_demo_runner.py\")\n",
        ...
        "source_path = os.path.join(PROJECT_DIR, \"spec-fastgs\", \"datasets\", \"mipnerf360\", \"counter\")\n",
        "model_path = os.path.join(PROJECT_DIR, \"spec-fastgs\", \"output\", \"demo_counter\")\n",
        "step3_dir = \"/kaggle/working/demo_video_counter/step3_training_snapshots\"\n",
        "fixed_cams_path = \"/kaggle/working/demo_video_counter/fixed_cameras.json\"\n",
        "\n",
        "with open(fixed_cams_path, \"r\", encoding=\"utf-8\") as f:\n",
        "    cam_data = json.load(f)\n",
        "selected_indices = cam_data[\"selected_indices\"]\n",
        "\n",
        "# Khai báo tham số huấn luyện chuẩn của Spec-fastgs\n",
        "parser = ModelParams()\n",
        "pipe_parser = PipelineParams()\n",
        "opt_parser = OptimizationParams()\n",
        "\n",
        "args = parser.extract([\n",
        "    \"-s\", source_path,\n",
        "    \"-m\", model_path,\n",
        ...
        "])\n",
        "pipe = pipe_parser.extract([])\n",
        "opt = opt_parser.extract([\"--iterations\", \"30000\", \"--densification_interval\", \"100\"])\n",
        "\n",
        "gaussians = GaussianModel(args.sh_degree)\n",
        ...
        "iter_start = 1\n",
        "for iteration in range(iter_start, opt.iterations + 1):\n",
        "    gaussians.update_learning_rate(iteration)\n",
        "    \n",
        "    # Chọn ngẫu nhiên viewpoint camera từ train set\n",
        ...
        "    loss = torch.abs(image - gt_image).mean()\n",
        "    loss.backward()\n",
        "    \n",
        "    with torch.no_grad():\n",
        ...
        "                gaussians.densify_and_prune(opt.densify_grad_threshold, 0.005, scene.cameras_extent, 50)\n",
        "                \n",
        "        gaussians.optimizer.step()\n",
        "        gaussians.optimizer.zero_grad(set_to_none=True)\n",
        "        \n",
        "    # Render và lưu snapshot mỗi 1.000 iterations (1k, 2k, ..., 30k)\n",
        "    if iteration % 1000 == 0 or iteration == opt.iterations:\n",
        "        print(f\"📸 [Iter {iteration:05d}/30000] Đang render 10 góc chụp cố định...\")\n",
        "        iter_dir = os.path.join(step3_dir, f\"iter_{iteration:05d}\")\n",
        "        os.makedirs(iter_dir, exist_ok=True)\n",
        "        \n",
        "        with torch.no_grad():\n",
        "            for i, cam in enumerate(selected_cameras):\n",
        "                r_pkg = render(cam, gaussians, pipe, bg_color)\n",
        "                img = r_pkg[\"render\"].clamp(0.0, 1.0)\n",
        "                np_img = (img.permute(1, 2, 0).cpu().numpy() * 255).astype(\"uint8\")\n",
        "                file_name = f\"view_{i+1:02d}_{cam.image_name}.png\"\n",
        "                Image.fromarray(np_img).save(os.path.join(iter_dir, file_name))\n",
        "        print(f\"  ✅ Đã lưu xong 10 snapshot cho Iteration {iteration}\")\n",
        "\n",
        "print(\"🎉 Huấn luyện và lưu Snapshots hoàn tất!\")\"\"\"\n",
        "\n",
        "with open(train_runner_script, \"w\", encoding=\"utf-8\") as f:\n",
        "    f.write(runner_code)\n",
        "\n",
        "# Chạy script huấn luyện\n",
        "import subprocess\n",
        "print(\"🚀 Tiến hành chạy huấn luyện demo và trích xuất snapshots...\")\n",
        "subprocess.run([sys.executable, train_runner_script], cwd=os.path.join(PROJECT_DIR, \"spec-fastgs\"), check=True)\n",
