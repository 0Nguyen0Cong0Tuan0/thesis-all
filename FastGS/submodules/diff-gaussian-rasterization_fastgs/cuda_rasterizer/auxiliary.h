...
__device__ const float SH_C0 = 0.28209479177387814f; // 1 / (2 * sqrt(pi))
__device__ const float SH_C1 = 0.4886025119029199f; // sqrt(3) * SH_C0
__device__ const float SH_C2[] = { 
	1.0925484305920792f, // sqrt(15) * SH_C0
	-1.0925484305920792f, // - sqrt(15) * SH_C0
	0.31539156525252005f, // sqrt(5) * SH_C0 / 2
	-1.0925484305920792f, // - sqrt(15) * SH_C0
	0.5462742152960396f  // sqrt(15) * SH_C0 / 2
};
...
{
  // v: NDC coordinate in [-1, 1] (with -1 being the left/top edge and 1 being the right/bottom edge)
  // S: screen size in that dimension (e.g., width for x, height for y)
	return ((v + 1.0) * S - 1.0) * 0.5; // Map [-1, 1] to [0, S-1]
}

__forceinline__ __device__ void getRect(const float2 p, int max_radius, uint2& rect_min, uint2& rect_max, dim3 grid)
{
  // Get the rectangular tile extent of a circle with center p and radius max_radius. 
  // The rectangle is clamped to the grid dimensions.
	rect_min = {
...
{
  // Get the rectangular tile extent of a rectangle with center p and half-extent ext_rect. 
  // The rectangle is clamped to the grid dimensions.

  // half-extent is used since the rectangle is centered at p, so the min corner is at p - ext_rect and the max corner is at p + ext_rect.
	rect_min = {
...
{
  // Used for computing the derivative of the normalized vector v with respect to z, where dv is the derivative of v with respect to z.
...
{
  // Used for computing the derivative of the normalized vector v with respect to v, where dv is the derivative of v with respect to v (i.e., the identity matrix).
	float sum2 = v.x * v.x + v.y * v.y + v.z * v.z;
	float invsum32 = 1.0f / sqrt(sum2 * sum2 * sum2);
...
{
  // Used for computing the derivative of the normalized vector v with respect to v, where dv is the derivative of v with respect to v (i.e., the identity matrix). This is for 4D vectors, which are used in some parts of the code for convenience. The w component is treated the same as the x, y, z components in the normalization, so the formula is similar to the 3D case but with an extra term for the w component.
...
// Used for checking if a point is in the view frustum. We check if the point is in front of the camera (i.e., has positive z in view space) and if it is within a certain distance from the camera (z > 0.2). We do not check the x and y bounds of the frustum here, since we will filter out points that are outside the frustum in the rasterizer anyway, and we want to avoid printing warnings for points that are outside the frustum but are expected to be filtered out.
__forceinline__ __device__ bool in_frustum(int idx,
	const float* orig_points, // orig_points is in world space
	const float* viewmatrix,  // viewmatrix transforms from world space to view space
	const float* projmatrix,  // projmatrix transforms from view space to clip space
	bool prefiltered, // If true, then points that are outside the frustum are expected to have been filtered out already, so we will not print a warning if we encounter such points. This is used for the second pass of the rasterizer, where we only process points that were not filtered out in the first pass.
	float3& p_view)
{
