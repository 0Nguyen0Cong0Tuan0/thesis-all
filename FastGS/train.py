...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, websockets, args=None):
    first_iter = 0

    # Prefer logging the full CLI Namespace (includes tracking flags). Keep dataset.model_path in sync.
    if args is not None:
    ...
    tracker = None
    if args is not None and getattr(args, "track_enable", False):
        tracker = TrainingTracker(
            ...
            render_func=render_fastgs,
        )
        tracker.write_hparams()
        tracker.init_fixed_views()
        tracker.log_event("init", first_iter, {"iterations": int(opt.iterations)})
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration % 1000 == 0:
            prev_degree = int(getattr(gaussians, "active_sh_degree", -1))
            gaussians.oneupSHdegree()
            new_degree = int(getattr(gaussians, "active_sh_degree", -1))
            if tracker is not None and new_degree != prev_degree:
                tracker.log_event("sh_degree_up", iteration, {"from": prev_degree, "to": new_degree})
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                if tracker is not None:
                    tracker.log_event("save_gaussians", iteration)
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    if tracker is not None:
                        tracker.log_event("densify_and_prune", iteration, {
                            "before": n_before,
                            "after": n_after,
                            "delta": n_after - n_before,
                            "densification_interval": int(opt.densification_interval),
                            "size_threshold": None if size_threshold is None else int(size_threshold),
                            "min_opacity": 0.005,
                        })
                ...
                if iteration % opt.opacity_reset_interval == 0 or (dataset.white_background and iteration == opt.densify_from_iter):
                    ...
                    if tracker is not None:
                        tracker.log_event("opacity_reset", iteration, {"opacity_reset_interval": int(opt.opacity_reset_interval)})
            ...
            if iteration % 3000 == 0 and iteration > 15_000 and iteration < 30_000:
                ...
                n_after = int(gaussians.get_xyz.shape[0])
                if tracker is not None:
                    tracker.log_event("final_prune", iteration, {
                        "before": n_before,
                        "after": n_after,
                        "delta": n_after - n_before,
                        "min_opacity": 0.1,
    ...
    if tracker is not None:
        tracker.log_event("train_end", int(opt.iterations), {"total_time_s": float(total_time)})
        tracker.flush_manifests()
...
def prepare_output_and_logger(args):    
    ...
    # Make model_path absolute early so later code doesn't depend on cwd.
