● Implemented the same tracking plugin for Specular-Gaussians:

   - Added D:\Thesis\All\Specular-Gaussians\tracking\ with:
    - hparams.json
    - runtime_changes.jsonl (per-iteration diffs)
    - events.jsonl (densify/anchor-adjust, opacity reset, SH degree up, saves, start/end)
    - fixed_views.json
    - renders_index.json + renders\<view>\<iter>.png (10 fixed views over training)
   - Integrated into:
    - Specular-Gaussians\train.py
    - Specular-Gaussians\train_anchor.py

  Enable with: --track_enable (plus --track_views, --track_render_interval, 
  --track_render_start/end, --track_save_gt_once). Outputs go under <model_path>\tracking\.
