...
def training(args, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint,
    ...
    def _get_lr(optimizer, name: str):
        for group in getattr(optimizer, "param_groups", []):
            if group.get("name") == name:
                try:
                    return float(group.get("lr"))
                except Exception:
                    return None
        return None

    tracker = None
    if getattr(args, "track_enable", False):
        try:
            from tracking import TrainingTracker
        except Exception as e:
            raise RuntimeError(f"Tracking enabled but tracking module could not be imported: {e}")

        def render_snapshot(cam, it: int):
            if getattr(dataset, "load2gpu_on_the_fly", False):
                cam.load2device()

            voxel_visible_mask = anchor_prefilter_voxel(cam, gaussians, pipe, background)
            image = anchor_render(
                cam,
                gaussians,
                pipe,
        ...
        tracker.write_hparams()
        tracker.init_fixed_views()
        tracker.log_event("train_start", 0, {"dataset_name": str(dataset_name)})
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                if tracker:
                    tracker.log_event("save", iteration)
            ...
            if iteration < opt.update_until and iteration > opt.start_stat:
                ...
                if iteration > opt.update_from and iteration % opt.update_interval == 0:
                    n_before = int(gaussians.get_anchor.shape[0])
                    ...
                    n_after = int(gaussians.get_anchor.shape[0])
                    if tracker:
                        tracker.log_event(
                            "adjust_anchor",
                            iteration,
                            {
                                "n_before": n_before,
                                "n_after": n_after,
                                "grad_threshold": float(opt.densify_grad_threshold),
                                "min_opacity": float(opt.min_opacity),
                                "success_threshold": float(opt.success_threshold),
                            },
                        )
            elif iteration == opt.update_until:
                if tracker:
                    tracker.log_event("update_until_cleanup", iteration)
            ...
            if tracker:
                visible_count = None
                try:
                    visible_count = int(voxel_visible_mask.sum().item())
                except Exception:
                    pass
                ...
                tracker.log_iteration(
                    iteration,
                    extra_state={
                        "retain_grad": bool(retain_grad),
                        "down_sampling": float(down_sampling),
                        "visible_count": visible_count,
                        "lr_anchor": _get_lr(gaussians.optimizer, "anchor"),
                        "lr_offset": _get_lr(gaussians.optimizer, "offset"),
                        "lr_mlp_opacity": _get_lr(gaussians.optimizer, "mlp_opacity"),
                        "lr_mlp_cov": _get_lr(gaussians.optimizer, "mlp_cov"),
                        "lr_mlp_color": _get_lr(gaussians.optimizer, "mlp_color"),
                    },
                    metrics={
                        "loss": float(loss.item()),
                        "Ll1": float(Ll1.item()),
                        "ssim_loss": float(ssim_loss.item()) if hasattr(ssim_loss, "item") else float(ssim_loss),
                        "scaling_reg": float(scaling_reg.item()) if hasattr(scaling_reg, "item") else float(scaling_reg),
                        "iter_ms": float(iter_start.elapsed_time(iter_end)),
                    },
                )

                if tracker.should_render(iteration):
                    tracker.render_fixed_views(iteration)

            if (iteration in checkpoint_iterations):
                logger.info("\n[ITER {}] Saving Checkpoint".format(iteration))
                torch.save((gaussians.capture(), iteration), scene.model_path + "/chkpnt" + str(iteration) + ".pth")
                if tracker:
                    tracker.log_event("checkpoint", iteration)


    if tracker:
        tracker.log_event("train_end", int(getattr(opt, "iterations", 0)))
        tracker.flush_manifests()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int,
                        default=[3_000, 7_000] + list(range(20000, 30001, 1000)))
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[3_000, 7_000, 30_000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[])
    parser.add_argument("--start_checkpoint", type=str, default=None)

    # Optional tracking (JSON/JSONL + fixed-view renders)
    parser.add_argument("--track_enable", action="store_true", default=False)
    parser.add_argument("--track_views", type=int, default=10)
    parser.add_argument("--track_render_interval", type=int, default=10)
    parser.add_argument("--track_render_start", type=int, default=1)
    parser.add_argument("--track_render_end", type=int, default=-1)
    parser.add_argument("--track_prefer_train", action="store_true", default=False)
    parser.add_argument("--track_save_gt_once", action="store_true", default=False)
    ...
    # training
    training(args, lp.extract(args), op.extract(args), pp.extract(args), dataset, args.test_iterations, args.save_iterations,
