...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    if getattr(args, "track_enable", False):
        ...
        def render_snapshot(cam, it: int):
            ...
            if it > 3000:
                ...
                if use_filter:
                    mlp_color = specular_mlp.step(
                        gaussians.get_asg_features[voxel_visible_mask],
            ...
            image = render(cam, gaussians, pipe, background, mlp_color, voxel_visible_mask=voxel_visible_mask)["render"]
        ...
        tracker.log_event("train_start", 0, {"use_filter": bool(use_filter)})
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if iteration % 1000 == 0:
            prev_deg = int(getattr(gaussians, "active_sh_degree", -1))
            gaussians.oneupSHdegree()
            if tracker:
                tracker.log_event(
                    "sh_degree_up",
                    iteration,
                    {"from": prev_deg, "to": int(getattr(gaussians, "active_sh_degree", -1))},
                )
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                    ...
                    if tracker:
                        tracker.log_event(
                            ...
                            iteration,
                            {
                                "n_before": n_before,
                                "n_after": n_after,
                                "max_grad": float(opt.densify_grad_threshold),
                                "min_opacity": 0.005,
                ...
                if iteration % opt.opacity_reset_interval == 0 or (
                    ...
                    if tracker:
                        tracker.log_event("opacity_reset", iteration)
            ...
            if iteration < opt.iterations:
                ...
                lr_specular = specular_mlp.update_learning_rate(iteration)

            lr_xyz = _get_lr(gaussians.optimizer, "xyz")
            if lr_specular is None:
                lr_specular = _get_lr(specular_mlp.optimizer, "specular")
            ...
            if tracker:
                ...
                tracker.log_iteration(
                    ...
                    extra_state={
                        "lr_xyz": lr_xyz,
                        "lr_specular": float(lr_specular) if lr_specular is not None else None,
                    ...
                    metrics={
                        ...
                        "Ll1": float(Ll1.item()),
                        "psnr_test": float(cur_psnr) if cur_psnr is not None else None,
    ...
    if tracker:
        tracker.log_event(
            "train_end",
            int(getattr(opt, "iterations", 0)),
            {
                "best_psnr": float(best_psnr),
                "best_iteration": int(best_iteration),
                "last_ssim": float(last_ssim),
                "last_lpips": float(last_lpips),
            },
        )
...
if __name__ == "__main__":
    ...
    training(args, lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations)
