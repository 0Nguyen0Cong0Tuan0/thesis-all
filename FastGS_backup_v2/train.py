...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, websockets):
    ...
    print(f"Training time: {training_time_seconds}")
    ...
    peak_vram_mib = round(peak_vram_bytes / (1024.0 * 1024.0), 2)

    # Format training time
    m, s = divmod(int(training_time_seconds), 60)
    h, m = divmod(m, 60)
    if h > 0:
        training_time_formatted = f"{h}h {m}m {s}s"
    else:
        training_time_formatted = f"{m}m {s}s"
...
def prepare_output_and_logger(args):    
    ...
    # Backup existing output folder if it exists and is not empty
    if os.path.exists(args.model_path) and os.listdir(args.model_path):
        ...
        try:
            shutil.move(args.model_path, backup_path)
            print(f"Existing output folder found. Backed up to: {backup_path}")
        except Exception as e:
            print(f"Warning: Could not backup existing output folder: {e}")
